...
namespace Sudoku.Client.Wrapper
{
        ...
        where T : class, IValidatableTrackingObject
    {
            ...
            : base(items)
        {
            ...
            AllTrackedItems = new ObservableCollection<T>(_originalCollection);
        }
        ...
        public ObservableCollection<T> AllTrackedItems { get; private set; }
        ...
        {
            //gets all items added to original collection
            var added = this.Where(current => _originalCollection.All(orig => orig != current));

            //gets all items removed from original collection
            ...
            SetObservableCollectionObjectsState();
        ...
        private void SetObservableCollectionObjectsState()
        {
            foreach (T item in _addedItems)
            {
            ...
            var allUnchanged = this.Except(_addedItems).Except(_removedItems).Except(_modifiedItems);
            foreach (var item in allUnchanged)
            {
            }
        }
